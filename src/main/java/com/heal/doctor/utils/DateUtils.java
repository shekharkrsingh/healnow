...
public class DateUtils {
    ...
    public static Date[] getStartAndEndOfDay(Date date) {
        try {
            ...
            LocalDate localDate = date.toInstant().atZone(istZoneId).toLocalDate();
            LocalDateTime startOfDay = localDate.atStartOfDay();
            LocalDateTime endOfDay = localDate.atTime(23, 59, 59, 999999999);
            Date startOfDayDate = java.util.Date.from(startOfDay.atZone(istZoneId).toInstant());
            Date endOfDayDate = java.util.Date.from(endOfDay.atZone(istZoneId).toInstant());
            return new Date[] {startOfDayDate, endOfDayDate};
        } catch (Exception e) {
    ...
    public static Date parseToDate(String dateString) {
        try {
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            LocalDate localDate = LocalDate.parse(dateString, formatter);
            return java.util.Date.from(localDate.atStartOfDay(ZoneId.of("Asia/Kolkata")).toInstant());
        } catch (Exception e) {
            throw new IllegalArgumentException("Invalid date format. Please use 'yyyy-MM-dd'.", e);
        }
    }
